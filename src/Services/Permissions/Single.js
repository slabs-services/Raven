export async function checkSendMailPermissionsSingle(req, outboxId, from, fromName, to, cc, bcc) {
    const roles = req.iamData.roles;
    if (!Array.isArray(roles)) {
        return { allowed: false, message: "Invalid Token (missing roles)" };
    }

    const canPut = roles.some((p) => p.fsId === "urn:slabs:iam:fs:raven:sendMail" && p.targetURN === outboxId);
